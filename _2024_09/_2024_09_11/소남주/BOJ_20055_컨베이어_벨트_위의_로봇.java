package _2024_09._2024_09_11.소남주;
...
2. 옮길 수 있다면 로봇 위치 한 칸씩 이동 -> 내구도 -1
    - 이동할 로봇이 없거나, 이동할 칸에 로봇이 이미 있거나, 내구도가 0이면 스킵
    - (N - 1)번 칸 이후로는 로봇이 존재할 수 없으므로, 이후는 고려하지 않아도 됨.
3. 0번째 칸에 로봇 올리기 -> 내구도 -1

- (N - 1)번 칸에 로봇이 도착하면 단계 관계없이 내리기
...
public class BOJ_20055_컨베이어_벨트_위의_로봇 {
    ...
    static void moveBelt() {
    	...
    	belt[N - 1] = false; // (N - 1)번 칸에 로봇이 있으면 내려야하고, 없더라도 false여야 하기 때문
